...
from models.task_state import TaskState, TaskState_Pydantic
...
async def get_task_state(
    ...
    return await TaskState_Pydantic.from_queryset(
        TaskState.filter(**query).offset(offset).limit(limit).order_by("-created")
    )
