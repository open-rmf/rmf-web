...
export interface RobotAlertDialogProps {
  alert: Alert;
  removeAlert: () => void;
}

export function RobotAlertDialog({ alert, removeAlert }: RobotAlertDialogProps): JSX.Element {
  ...
  const [robotAlert, setRobotAlert] = React.useState<RobotAlert | null>(null);
  ...
  React.useEffect(() => {
    ...
    if (!names || names.length !== 2) {
      console.log(
        `Failed to retrieve fleet and robot name from alert ${alert.original_id}
        `,
      );
      return;
    }
    ...
    const sub = rmf.getFleetStateObs(fleet).subscribe(async (fleetState) => {
  ...
  const acknowledgeAlert = () => {
    ...
    (async () => {
      const ackResponse = (
        await rmf?.alertsApi.acknowledgeAlertAlertsAlertIdPost(robotAlert.originalId)
      ).data;
      if (ackResponse.id !== ackResponse.original_id) {
        ...
        console.log(`Failed to acknowledge alert ID ${robotAlert.originalId}`);
        showAlert('error', `Failed to acknowledge alert ID ${robotAlert.originalId}`);
      }
    })();
  };
  ...
  return (
    <>
      {robotAlert.show ? (
        robotAlert.acknowledgedBy ? (
          <AlertDialog
            key={robotAlert.originalId}
            onDismiss={removeAlert}
            acknowledgedBy={robotAlert.acknowledgedBy}
            title={getAlertTitle(robotAlert)}
            alertContents={getAlertContent(robotAlert)}
            backgroundColor={getAlertColor(robotAlert)}
          />
        ) : (
          <AlertDialog
            key={robotAlert.originalId}
        ...
        <CloseAlertDialog title={getAlertTitle(robotAlert)} />
      )}
    </>
  );
}
