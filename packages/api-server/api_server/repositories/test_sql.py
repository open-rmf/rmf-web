...
class TestSqlRepository(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_read_door_states(self):
        door_state = test_data.make_door_state("test_door", RmfDoorMode.MODE_CLOSED)
        door_state.door_time = RosTime(sec=0, nanosec=0)
        door_state2 = test_data.make_door_state("test_door2", RmfDoorMode.MODE_CLOSED)
        door_state.door_time = RosTime(sec=0, nanosec=0)
    ...
    async def test_update_door(self):
        door = test_data.make_door("test_door")
    ...
    async def test_sync_doors(self):
        ...
        door2 = test_data.make_door("test_door2")
    ...
    async def test_update_lift_state(self):
        lift_state = test_data.make_lift_state()
        ...
        lift_state2 = test_data.make_lift_state()
    ...
    async def test_update_dispenser_state(self):
        ...
        state_2 = test_data.make_dispenser_state("test_dispenser_2")
    ...
    async def test_update_fleet_state(self):
        ...
        state_2 = test_data.make_fleet_state("test_fleet_2")
        state_2.robots = []
        ...
        state_2.robots = [test_data.make_robot_state("test_robot_2")]
        await self.repo.update_fleet_state(state_2)
        result: RmfFleetState = await self.repo.read_fleet_states()
        self.assertEqual(len(result), 2)
