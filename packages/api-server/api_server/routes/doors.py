...
class DoorsRouter(FastIORouter):
    def __init__(self, app: BaseApp):
        ...
        async def get_door_state(door_name: str):
            ...
            """
            return await app.rmf_repo.get_door_state(door_name)

        @self.watch("/{door_name}/state")
        async def watch_door_state(req: WatchRequest, door_name: str):
            await req.emit(await get_door_state(door_name))
            rx_watcher(
                req,
        ...
        async def get_door_health(door_name: str):
            """
            Available in socket.io
            """
            return await app.rmf_repo.get_door_health(door_name)

        @self.watch("/{door_name}/health")
        async def watch_door_health(req: WatchRequest, door_name: str):
            await req.emit(get_door_health(door_name))
            rx_watcher(
                req,
